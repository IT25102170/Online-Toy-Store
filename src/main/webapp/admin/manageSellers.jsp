        ...
        <div class="bg-gray-900 rounded-xl shadow-lg p-6 mb-6 transition-transform transform hover:scale-105 max-w-3xl mx-auto">
            ...
            <form action="${pageContext.request.contextPath}/sellers" method="post" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input type="text" name="name" value="${currentSeller.name}" placeholder="Name" class="w-full px-4 py-2 rounded-lg bg-gray-800 text-white border border-gray-600 focus:outline-none" required />
                    <input type="email" name="email" value="${currentSeller.email}" placeholder="Email" class="w-full px-4 py-2 rounded-lg bg-gray-800 text-white border border-gray-600 focus:outline-none" required />
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input type="password" name="password" placeholder="Password" class="w-full px-4 py-2 rounded-lg bg-gray-800 text-white border border-gray-600 focus:outline-none" <c:if test="${mode == 'create'}">required</c:if> />
                    <input type="text" name="storeName" value="${currentSeller.storeName}" placeholder="Store Name" class="w-full px-4 py-2 rounded-lg bg-gray-800 text-white border border-gray-600 focus:outline-none" required />
                </div>
    ...
    <c:if test="${mode == 'list'}">
        <div class="bg-gray-900 rounded-xl shadow-lg overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-700 flex justify-between items-center">
                ...
                <a href="${pageContext.request.contextPath}/sellers?mode=create" class="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-md text-sm font-medium">
                    + Add Seller
                </a>
            </div>
